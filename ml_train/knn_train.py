...
def test(knn_human, knn_act, test_dataloader, person_classes, act_classes, device, val_data_num, args, scaler):
    ...
    with torch.no_grad():
        ...
        human_hat = np.array(knn_human.predict_proba(test_features))
        human_hat = np.argmax(human_hat, axis=-1)

        act_hat = np.array(knn_act.predict_proba(test_features))
...
if __name__ == "__main__":
    ...
    result_folder = os.path.join(os.getcwd(), f'knn_human_act')
    ...
    knn_human = KNeighborsClassifier(n_neighbors=18, p=2)
    knn_human.fit(train_features, train_human_labels)
   
    knn_act = KNeighborsClassifier(n_neighbors=20, p=2)
    knn_act.fit(train_features, train_act_labels)

    test(knn_human, knn_act, val_dataloader, person_classes, act_classes, device, val_data_num, args, scaler)
